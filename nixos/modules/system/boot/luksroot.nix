  ...
  in ''
        ...
        while true; do
            ...
            while true; do
                if [ -e /crypt-ramfs/passphrase ]; then
                    echo "Passphrase for ${dev.device} reused"
                    passphrase=$(cat /crypt-ramfs/passphrase)
                    break
                else
                    echo -n "${dev.device}" > /crypt-ramfs/device
                    passphrase="$(askPassword "Passphrase for ${dev.device}: ")"
    ...
    do_open_yubikey() {
        ...
        for try in $(seq 3); do
            ...
            while true; do
                if [ -e /crypt-ramfs/passphrase ]; then
                    echo "Two-factor passphrase reused"
                    k_user=$(cat /crypt-ramfs/passphrase)
                    break
                else
                    k_user="$(askPassword "Two-factor passphrase: ")"
                    if [ -n "$k_user" ]; then
                       ${if luks.reusePassphrases then ''
                         # Remember it for the next device
    ...
    do_open_gpg_card() {
        ...
        for try in $(seq 3); do
            ...
            while true; do
                if [ -e /crypt-ramfs/passphrase ]; then
                    echo "PIN for GPG Card associated with device ${dev.device} reused"
                    pin=$(cat /crypt-ramfs/passphrase)
                    break
                else
                    pin="$(askPassword "PIN for GPG Card associated with device ${dev.device}: ")"
                    if [ -n "$pin" ]; then
                       ${if luks.reusePassphrases then ''
                         # remember it for the next device
    ...
    open_with_hardware() {
      local passsphrase
        ...
        ${if dev.fido2.passwordLess then ''
          export passphrase=""
        '' else ''
          passphrase="$(askPassword "FIDO2 salt for ${dev.device}: ")"
        ''}
